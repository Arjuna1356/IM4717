...
@ $db = new mysqli('localhost', 'f37ee', 'f37ee', 'f37ee');
  ...
  $check = $_POST['button'];
  $query = $db->prepare("SELECT movieName, movieDate, movieTime from movies where movieID= '$check'");
  $query->bind_result($movieName, $movieDate, $movieTime);
  $query->execute();
  $query->fetch();
  $query->close();

 // $query4 = "SELECT seatID from order_items where movieID="$movieID"";
 //  $taken = array();
 //
 //  while ($unav=$db->query($query4)) {
 //    $taken[]=$unav;
 //  }
    ...
    <form name="bookingform" action="update.php" method="post">
      <table onload="seatNA();" >
        <tr>
          ...
          <td><input type="hidden" name="movieID" value="<?php echo $check; ?>"></td>
        </tr>
        <tr>
          <td><b>Email:</b></td>
        ...
        <tr>
          <td><input type="submit" name="makebooking" value="Make Booking"></td>
        </tr>
      </table>
    </form>
